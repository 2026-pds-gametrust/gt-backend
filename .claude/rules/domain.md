...
- **Forbidden in service:** import `src/infraestructure`, `mongoose`, `*Model`, `IM*` models, concrete SQS producers.
...
- Messaging (optional): when added, keep **interfaces** only in `domain/<context>/messaging/<event>/` (e.g. `producer.interface.ts`); implementations go in infraestructure.
