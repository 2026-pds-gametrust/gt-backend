...
| Domain | Method | Path | Summary | Folder |
|---------|--------|------|--------|-------|
...
```text
docs/en/api/<module>/<resource>/<method-path>/
...
1. **Trust > volume** — never show a seal/verification without a completed API status.
2. **Product ≠ Offer** — `/products` is the model; `/listings` is the unit/offer.
3. **AI does not invent** — attributes come from `attribute-schema` and the response; do not fill gaps.
4. **TrustScore with reasons** — use returned events/score; do not reduce to a color without text.
5. **Identity only in the JWT** — never spoof `x-user-id` / `x-user-groups`.
6. **Public signup** is `POST /auth/register`, not `POST /users` (ADMIN).
