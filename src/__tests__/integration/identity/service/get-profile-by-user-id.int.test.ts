...
describe('when we get a profile by user id', () => {
  it('should return the profile for that user', async () => {
    ...
    const result = await profileService.getProfileByUserId(user.id);
...
describe('when we get a profile by a user id without profile', () => {
  it('should reject with RESOURCE_NOT_FOUND', async () => {
    const missingUserId = new Types.ObjectId().toHexString();

    await expect(
      profileService.getProfileByUserId(missingUserId),
